...
class AddMoreUniqueIndexes < ActiveRecord::Migration
  def change
    ...
    add_index :api_subject_role_assignments, %i(api_subject_id role_id),
              unique: true
    add_index :available_attributes, %i(name value), unique: true
    add_index :permitted_attributes, %i(provider_id available_attribute_id),
              unique: true, name: 'permitted_attributes_unique_attribute'
    add_index :provided_attributes, %i(subject_id permitted_attribute_id),
              unique: true, name: 'provided_attributes_unique_attribute'
    add_index :providers, :identifier, unique: true
    add_index :subject_role_assignments, %i(subject_id role_id), unique: true
